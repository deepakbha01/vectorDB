...
import { InfrastructureDesignService } from './infrastructure.service';
import { CreateInfrastructureDesignDto } from './create-infrastructure-design.dto';

/** Infrastructure Decision Record (spec §9). Hidden unless AI_FACTORY_ENABLED is on. */
@ApiTags('ai-factory')
@ApiBearerAuth()
@UseGuards(AiFactoryEnabledGuard, JwtAuthGuard, RolesGuard)
@Controller('projects/:projectId/ai-factory/infrastructure')
export class InfrastructureDesignController {
  constructor(private readonly service: InfrastructureDesignService) {}

  /** Placement context derived from the latest records, plus a preview design. Saves nothing. */
  @Get('defaults')
  ...
  submit(@Param('projectId', ParseUUIDPipe) projectId: string, @CurrentUser() user: AuthenticatedUser, @Body() dto: CreateInfrastructureDesignDto) {
  ...
  async latest(@Param('projectId', ParseUUIDPipe) projectId: string, @CurrentUser() user: AuthenticatedUser) {
    ...
    if (!d) throw new NotFoundException('No infrastructure design has been made for this project yet.');
    return d;
  }
}
